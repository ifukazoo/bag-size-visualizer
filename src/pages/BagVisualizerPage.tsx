...
export default function BagVisualizerPage() {
  ...
  return (
    // 狭幅では縦1カラム、lg 以上では「操作パネル ｜ キャンバス」の2カラム。
      ...
      <div className="flex flex-col gap-5">
        <div className="flex flex-col gap-2">
          ...
          <button
            ...
            className="flex items-center min-h-11 border border-brand-border rounded-soft px-4 py-2 text-sm text-brand-sumi hover:border-brand-indigo transition-colors text-left disabled:opacity-50 disabled:cursor-not-allowed"
          >
            自動で背景透過して使う
          </button>
          <button
        ...
        <div className="flex flex-wrap gap-x-6 gap-y-4">
          <div className="flex flex-col gap-1">
            ...
            <input
              ...
              className="min-h-11 border border-brand-border rounded-soft px-3 py-2 text-sm text-brand-sumi w-28 focus:outline-none focus:border-brand-indigo"
            />
          </div>
        ...
        <button
          onClick={downloadPng}
          className="inline-flex items-center justify-center min-h-11 border border-brand-indigo rounded-soft px-4 py-2 text-sm text-brand-indigo hover:bg-brand-indigo hover:text-white transition-colors"
        >
          PNG ダウンロード
        </button>
        ...
        {bgStatus === 'error' && (
          <p className="text-red-500 text-xs">{bgError}</p>
        )}
      </div>
